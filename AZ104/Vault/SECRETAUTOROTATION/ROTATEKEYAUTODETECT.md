...
.
├── app/
│   ├── app.py               # Flask app: reads the secret, listens for rotation
│   └── requirements.txt
├── deploy/
│   └── kvdemo.service       # systemd unit
├── rotator/
│   ├── function_app.py      # Azure Function that rotates the secret
│   ├── host.json
│   └── requirements.txt
├── scripts/
│   └── rotate-now.sh        # Manual rotation for the demo
└── .github/workflows/
    └── deploy.yml           # CI/CD to the VM
```
...
#!/usr/bin/env bash
set -euo pipefail
: "${KV:?set KV}" "${SECRET_NAME:=app-api-key}"
az keyvault secret set --vault-name "$KV" -n "$SECRET_NAME" \
  --value "$(openssl rand -base64 24)" \
  --expires "$(date -u -d '+90 days' '+%Y-%m-%dT%H:%M:%SZ')" \
  --tags rotationDays=90 \
...
```bash
chmod +x scripts/rotate-now.sh && ./scripts/rotate-now.sh
```
